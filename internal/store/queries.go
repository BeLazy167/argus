...
func (s *Store) ListReviews(ctx context.Context, repoID int64, limit, offset int) ([]Review, error) {
	...
	rows, err := s.Pool.Query(ctx, `
		SELECT id, repo_id, pr_number, pr_title, pr_author, head_sha, base_sha, github_review_id,
		       status, summary, score, trigger, triggered_by, duration_ms, error, created_at, completed_at
...
func (s *Store) GetReview(ctx context.Context, id uuid.UUID) (*Review, error) {
	...
	`, id).Scan(&r.ID, &r.RepoID, &r.PRNumber, &r.PRTitle, &r.PRAuthor, &r.HeadSHA, &r.BaseSHA, &r.GithubReviewID,
		&r.Status, &r.Summary, &r.Score, &r.Trigger, &r.TriggeredBy, &r.DurationMs, &r.Error, &r.CreatedAt, &r.CompletedAt)
	if err != nil {
		return nil, err
	}
	return &r, nil
...
func (s *Store) GetReviewComments(ctx context.Context, reviewID uuid.UUID) ([]ReviewComment, error) {
	rows, err := s.Pool.Query(ctx, `
		SELECT id, review_id, file_path, start_line, end_line, side, body, severity, category, created_at
...
func (s *Store) CreateReviewComment(ctx context.Context, reviewID uuid.UUID, filePath string, startLine, endLine *int, side *string, body string, severity, category *string) error {
	_, err := s.Pool.Exec(ctx, `
		INSERT INTO review_comments (review_id, file_path, start_line, end_line, side, body, severity, category)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
	`, reviewID, filePath, startLine, endLine, side, body, severity, category)
	return err
}
