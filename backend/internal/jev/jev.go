...
func (c *Client) Evaluate(ctx context.Context, state any, questions map[string]Question, stage string) (Result, error) {
	...
	} else if len(raw) > maxStateBytes {
		return Result{}, fmt.Errorf("state too large: %d bytes > %d", len(raw), maxStateBytes)
	}
	body := evalRequest{Model: c.model, State: state, Questions: questions}
	...
	res, statusCode, err := c.evaluate(ctx, operationID, body)
...
// evaluate is the untyped inner call (telemetry lives in Evaluate). Returns
// the parsed result, the HTTP status (0 for pre-response failures), and error.
func (c *Client) evaluate(ctx context.Context, operationID string, body evalRequest) (Result, int, error) {
	jsonBody, err := json.Marshal(body)
	if err != nil {
		return Result{}, 0, fmt.Errorf("marshaling request: %w", err)
	}
	...
	if resp.StatusCode != http.StatusOK {
		return Result{}, resp.StatusCode, fmt.Errorf("jev API error (status %d): %s", resp.StatusCode, string(respBody))
	}
