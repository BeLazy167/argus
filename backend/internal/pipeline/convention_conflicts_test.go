...
func TestConventionConflictCorpusOwned(t *testing.T) {
	// The corpus is built from real reviews of private repositories, so it is
	// not distributed with the open-source tree. Skip rather than fail when it
	// is absent; the rest of this file's tests need no fixture.
	raw, err := os.ReadFile("testdata/convention-conflict-corpus.json")
	if errors.Is(err, os.ErrNotExist) {
		t.Skip("convention-conflict corpus not present in this checkout")
	}
	if err != nil {
		t.Fatal(err)
	}
