...
import type { InstallationContextType } from "./context";

/**
 * Loads the caller's installations and the org-scoped current installation,
 * auto-linking a matching installation to the active Clerk org when needed,
 * and derives the InstallationContext value.
 *
 * Returns:
 *   The InstallationContextType value to feed InstallationContext.Provider.
 */
export function useInstallations(): InstallationContextType {
	...
	useEffect(() => {
		...
		const match =
			...
			) ?? installations.find((i) => i.clerk_org_id === orgId);
		if (!match) return;
