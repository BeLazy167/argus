...
import { Activity, ExternalLink, FileText, Loader2, Search } from "lucide-react";
...
export function InsightsSection() {
	...
	const { data: traces, isLoading: tracesLoading } = useTraces(fileFilter || undefined);
	...
	const filteredRisks = useMemo(() => {
		...
		return risks.filter((r) => r.file_path.toLowerCase().includes(fileFilter.toLowerCase()));
	}, [risks, fileFilter]);
						...
						{tracesLoading ? (
							...
							<div className="divide-y divide-iron/30">
								{tracePaginated.map((trace) => (
									<div key={trace.id} className="px-5 py-3 hover:bg-iron/10 transition-colors">
