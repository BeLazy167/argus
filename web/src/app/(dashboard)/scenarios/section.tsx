...
function VerdictBadge({ verdict }: { verdict: ScenarioVerdict }) {
	return (
		<span
			className={`inline-block rounded border px-2 py-0.5 text-[11px] font-mono ${VERDICT_BADGE[verdict]}`}
		>
...
function ScenarioDrawer({ scenario }: { scenario: Scenario }) {
	...
	const verdict = scenario.last_verdict;
	...
	return (
		<div className="mt-4 border-t border-iron/30 pt-4 space-y-4">
			...
			<div className="flex items-center gap-3 text-[11px] font-mono text-slate-text flex-wrap">
				<span>
					Triggered <span className="text-foreground">{scenario.trigger_count ?? 0}</span> times
				</span>
			...
			<div className="space-y-2">
				...
				{isLoading ? (
					...
					<ul className="space-y-1.5">
						{runs.map((run) => (
							<li key={run.id} className="border border-iron/40 bg-iron/5 px-3 py-2 space-y-1">
								<div className="flex items-center gap-2 flex-wrap text-[11px] font-mono">
								...
								{run.why && (
									<p className="text-[11px] font-mono text-foreground leading-relaxed">{run.why}</p>
								)}
								{run.fix && (
...
/**
 * KpiCard — one of the four summary tiles at the top of the section. Clicking filters the
...
function KpiCard({
	...
	return (
		...
		>
			...
			<span
				className={`font-mono text-2xl ${toneClass}`}
				title={unavailable ? "KPI fetch failed" : undefined}
			>
				{unavailable ? "—" : count}
			</span>
		</Wrapper>
		...
		<ProGate feature="Scenario memory">
          ...
          the counts are zero. The scenario list still renders independently. */}
			<div className="mb-4 grid grid-cols-2 md:grid-cols-4 gap-3">
				...
				<KpiCard
					label="Broken this week"
					count={kpis?.broken_this_week ?? 0}
					tone="red"
					unavailable={kpisError}
				/>
				<KpiCard
					label="Fixed this week"
					count={kpis?.fixed_this_week ?? 0}
					tone="emerald"
					unavailable={kpisError}
				/>
				<KpiCard
					label="Outdated"
					count={kpis?.outdated ?? 0}
					tone="yellow"
					unavailable={kpisError}
				/>
			</div>
				...
				<div className="border border-amber/30 bg-charcoal p-5 space-y-5">
					<form onSubmit={handleCreate} className="space-y-5">
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
							<div>
								<span className="block text-[11px] font-mono uppercase tracking-wider text-slate-text mb-1.5">
									Severity
								</span>
								<div className="flex">
				...
				{isLoading ? (
					...
					<div className="py-16 flex flex-col items-center gap-4">
						...
						<div className="text-center">
							...
							<p className="text-xs font-mono text-slate-text max-w-xs">
								Argus auto-creates scenarios from critical findings in reviews, or you can add one
								manually.
							</p>
						</div>
						<button
					...
					<div className="divide-y divide-iron/30">
						{paginated.map((scenario) => {
							...
							const toggle = () => setExpandedId(isExpanded ? null : scenario.id);
							return (
								<div key={scenario.id} className="flex hover:bg-iron/10 transition-colors">
									{/* Severity bar */}
									<div
										className={`w-1 shrink-0 ${SEVERITY_DOT[scenario.severity] ?? SEVERITY_DOT.medium}`}
									/>
									...
									<div className="flex-1 min-w-0 px-5 py-4">
										<div className="flex items-start justify-between gap-4">
											<button
												...
												className="flex-1 min-w-0 text-left cursor-pointer focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-amber/50"
											>
												{/* Row 1: title */}
												<div className="flex items-start gap-1.5">
													...
													/>
													<span
														className={`block text-sm font-mono text-foreground leading-snug ${
