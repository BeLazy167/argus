  ...
  return (
    <article className="border-b border-iron/60 px-5 py-4 last:border-0">
      ...
      <div className="mt-3 flex items-center gap-3 font-mono text-[10px] text-slate-text">
        ...
        {memory.review_id ? (
          <Link className="ml-auto shrink-0 text-amber hover:underline" href={`/reviews/${memory.review_id}`}>
            Source review
          </Link>
        ) : null}
      </div>
  ...
  return (
    <section aria-labelledby="memories-heading">
      <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          ...
          <select
            ...
            value={scope}
            onChange={(event) => updateFilter({ memory_scope: event.target.value === "all" ? "" : event.target.value })}
      ...
      <div className="border border-iron bg-charcoal">
        {isLoading && !data ? (
          <div role="status" className="flex items-center justify-center gap-2 py-16 font-mono text-xs text-slate-text">
