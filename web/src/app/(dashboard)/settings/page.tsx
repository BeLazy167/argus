...
const PERSONAS = [
	...
	},
	{
		...
		label: "Strict",
		description:
			"Exhaustive analysis depth — traces every path and error branch; severity bar unchanged",
	},
	{
		value: "custom",
		label: "Custom",
...
function StatusBadge({ variant, label }: { variant: BadgeVariant; label: string }) {
	...
	return (
		<span
			className={`inline-flex items-center rounded-sm border px-1.5 py-0.5 text-[9px] font-mono uppercase tracking-wider ${styles[variant]}`}
		>
			{label}
		</span>
	);
		...
		<div className="border border-amber/30 bg-amber/5 px-4 py-3">
			<div className="flex items-start gap-2.5">
				...
				<p className="text-[11px] font-mono text-amber/80">
					Org defaults missing for{" "}
					<span className="text-amber font-medium">{missingStages.join(", ")}</span> — repos without
					their own config can&apos;t run these stages.
				</p>
			</div>
			<div className="mt-3 flex flex-wrap items-end gap-3 pl-6">
				<div>
					<label
						htmlFor="org-gap-provider"
				...
				>
					<Save className="h-3 w-3" />
					<span role="status" aria-live="polite">
						{isSubmitting ? "Applying..." : "Apply to all stages"}
					</span>
				</button>
			</div>
			{picks.length > 0 && (
			...
			{error && (
				<p role="alert" className="mt-2 pl-6 text-[10px] font-mono text-red-400">
					{error}
				</p>
			)}
		</div>
						...
						<div className="space-y-10">
							<div className="border border-amber/20 bg-amber/5 px-4 py-3 flex items-start gap-2.5">
								...
								<p className="text-[11px] font-mono text-amber/80">
									...
									<span className="text-amber font-medium">{active?.org_login}</span>. Repos can
									override individual settings on the &quot;Repo Overrides&quot; tab.
								</p>
							</div>
								...
								{orgPersona === "custom" && (
									<div className="mt-4 border border-iron bg-charcoal p-4">
										...
										<button
											type="button"
											disabled={
												saveOrgDefaults.isPending || orgCustomPromptDraft === orgCustomPrompt
											}
											onClick={() => {
							...
							<section>
								...
								<p className="text-xs font-mono text-slate-text mb-4">
									Configure the default model for each review stage. Applies to all repos unless
									overridden.
								</p>
							...
							<section>
								...
								<p className="text-xs font-mono text-slate-text mb-4">
									Default for all repos in {active?.org_login}. Turn off to require opt-in per PR
									via a checkbox comment with cost preview.
								</p>
								<PipelineFeatureCard
			...
			{settingsScope === "repo" &&
				(loading ? (
					...
					<div className="border border-iron bg-charcoal p-10 text-center">
						...
						<p className="text-xs font-mono text-slate-text">No installation found.</p>
					</div>
				) : (
					<div className="space-y-10">
						<div className="border border-iron/50 bg-iron/10 px-4 py-3 flex items-start gap-2.5">
						...
						<section>
							<div className="flex items-center gap-3 mb-4">
								...
								<div className="flex items-center gap-2">
									...
									<h2 className="font-mono text-lg font-semibold text-foreground">API Keys</h2>
								</div>
							...
							{configuredCount === 0 ? (
								...
								<p className="text-[11px] font-mono text-slate-text mb-1">
									...
									<span className="text-foreground">
										{repos.find((r) => r.id === activeId)?.full_name ?? "selected repo"}
									</span>
									.
								</p>
							)}
						...
						<section>
							...
							<p className="text-[11px] font-mono text-slate-text mb-4">
								...
								<code className="rounded bg-iron/50 px-1 py-0.5 text-amber">
										...
										<>
											<div className="mt-4 border border-iron bg-charcoal p-4">
												...
												<button
													type="button"
													disabled={
														updateRepo.isPending || customPromptDraft === currentCustomPrompt
													}
													onClick={() => {
														setPersonaError("");
														updateRepo.mutate(
															{
																id: activeId,
																settings_json: {
																	...activeRepo?.settings_json,
																	persona: "custom",
																	custom_persona_prompt: customPromptDraft,
																},
															},
															{
																onError: (err) =>
						...
						<section>
							...
							<p className="text-[11px] font-mono text-slate-text mb-4">
								Repo override for{" "}
								<span className="text-foreground">{activeRepo?.full_name ?? "selected repo"}</span>.
								When off, Argus posts a trigger checkbox on opened PRs instead of running
								automatically.
							</p>
							...
							{activeId === 0 ? (
								...
								<>
									<PipelineFeatureCard
										toggle={AUTO_RUN_TOGGLE}
										enabled={
											typeof activeRepo?.settings_json?.auto_run === "boolean"
									...
									<PipelineFeatureCard
										toggle={AUTO_RESOLVE_TOGGLE}
										enabled={
											typeof activeRepo?.settings_json?.auto_resolve_enabled === "boolean"
						...
						<>
							<section>
								...
								<p className="text-[11px] font-mono text-slate-text mb-4">
									...
									<span className="text-foreground">
										{activeRepo?.full_name ?? "selected repo"}
									</span>
											...
											<form
												...
												onSubmit={(e) => {
													...
													updateRepo.mutate({
														...
														settings_json: {
															...settings,
															skip_base_branches: [...skipBranches, val],
														},
													});
													input.value = "";
												}}
