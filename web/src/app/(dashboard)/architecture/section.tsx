...
export function ArchitectureSection() {
	...
	// Remount the view when the active repo changes so per-repo state (the open
	// file sidebar, lens, search) can't leak a stale path across repos. Keyed
	// remount instead of an effect.
	return <ArchitectureView key={activeId} activeId={activeId} />;
}

function ArchitectureView({ activeId }: { activeId: number }) {
			...
			{showGuide && archData && archData.files.length > 0 && (
				<div className="px-5 py-2 border-b border-iron bg-card/50 flex items-start gap-3 shrink-0">
					...
					<div className="text-[10px] font-mono text-slate-400 leading-relaxed space-y-0.5">
						<p>
							Node height scales with risk; width fits the file name. Border color = bug density (
						...
						<p>
							Click a node to inspect. Switch lenses above to highlight choke points, hotspots, or
							coupling.
						</p>
					</div>
					<button
