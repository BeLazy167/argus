...
export const competitors: Competitor[] = [
  ...
  },
  {
    ...
    pricing: "Usage-based — code review $0.05/KB of diff (10 KB min); status $0.05/commit; $100 free credit to start; no per-seat fees; Enterprise annual discounts",
    strengths: [
      "Precision / low-noise focus — advertises ~4x less noise, with per-review and per-PR spend caps",
      "AST graph-based representation of the whole codebase for cross-file context",
      "Approvability auto-approves low-risk PRs; a Fix-It agent runs CI and iterates on its fix until tests pass",
      "Context-aware: pulls the linked Jira / Linear ticket, Sentry errors, and LaunchDarkly flags into the review",
      "Learns from thumbs up/down feedback to calibrate future reviews; the Agent can also create Jira / Linear tickets and draw architecture / sequence diagrams",
      "Usage-based pricing with no per-seat fees; SOC 2 Type II certified",
    ],
    weaknesses: [
      "GitHub-only — no GitLab, Bitbucket, or Azure DevOps",
      "No self-hosted / on-prem deployment (SaaS only)",
      "Single-pass review rather than a multi-specialist pipeline",
      "No failure-scenario simulation, bundled SAST, test generation, BYOK, or per-PR review contract",
    ],
    argusAdvantage:
      "Macroscope and Argus both chase signal-to-noise, and both learn from team feedback. Macroscope pushes harder on workflow automation: it auto-approves low-risk PRs, runs CI to self-heal its own fixes, and can create Jira / Linear tickets — none of which Argus does today — plus a SOC 2 Type II certification Argus doesn't have. It's GitHub-only and SaaS-only, though. Argus's distinct edges are the review's depth and control: a multi-pass specialist pipeline, failure-scenario simulation, a computed per-PR review contract that routes depth automatically, BYOK, self-hosting, and a Glass Box footer that shows what was checked and suppressed. Pick Macroscope for hands-off automation (auto-approve + auto-fix) on GitHub with usage-based pricing; pick Argus for a deeper, self-hostable reviewer with depth routing and cost control.",
    summary:
      "Macroscope is a precision, low-noise AI reviewer for GitHub that leans into automation: Approvability auto-approves low-risk PRs, a Fix-It agent runs CI and self-heals its fix until tests pass, and it pulls Jira / Sentry / LaunchDarkly context into reviews — all on usage-based pricing with no per-seat fees. It builds an AST graph of the codebase, learns from thumbs up/down feedback, creates tickets and diagrams, and is SOC 2 Type II. It's GitHub-only and SaaS-only, with no multi-pass specialist pipeline, failure simulation, BYOK, self-hosting, or per-PR review contract.",
    stat: {
      claim: "Macroscope advertises ~4x less noise than other AI reviewers and prices per usage ($0.05/KB reviewed) with no per-seat fees",
