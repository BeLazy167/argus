...
| Postgres version? | **Two-track:** dev+CI on 19beta2 (spike-verified: pgvector 0.8.6 from master builds and the full design SQL passes); prod cutover takes 19-GA-if-released-else-18; self-host default `pgvector/pgvector:pg18`. See 01 §Postgres version policy. |
