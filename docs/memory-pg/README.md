...
| Replace with Polygres (pgContext/pgGraph)? | **Not now.** PG 17/18-only (prod runs 17.7; the self-host compose default is 16), superuser + custom access method makes it structurally impossible on every managed Postgres (RDS/Supabase/Neon/Cloud SQL) — locking out self-hosters; 11 days old, single maintainer; ANN + fusion features are self-labeled Experimental; its hybrid RRF score breaks the 0.85-similarity suppression contract. **Evaluated in depth and DECLINED 2026-08-02** (see 01-replacement-architecture.md). The old "drop-on-top upgrade" hedge was FALSE: registration gates on `atttypid = 'pgcontext.vector'::regtype` while migration 059 gives us `public.vector(1024)`, so adoption needs a retype migration below the Indexer seam. Its fused hybrid entry point also takes no filter (cross-tenant read) and discards cosine (suppression fails silent-open). |
| Replace with what? | **pgvector + core Postgres FTS** (embeddings: voyage-4 @ 1024 dims, SOTA-validated; OpenAI BYOK truncates to 1024) in the existing database. Available on Fly MPG, RDS, Supabase, Neon, and via a one-line docker-compose change. Preserves the [0,1] similarity score contract exactly. Net-negative complexity: deletes the rate limiter, backoff, BYOK client registry, batch collision-merge workarounds, and the cron decay phase. |
...
| Postgres version? | **Two-track:** dev+CI on 19beta2 (spike-verified: pgvector 0.8.6 from master builds and the full design SQL passes); prod already runs PG 17.7 + pgvector 0.8.5 (db name: postgres); self-host compose default `pgvector/pgvector:pg16` now (volume-safe), major bump at prod cutover. See 01 §Postgres version policy. |
