...
See `/Users/belazy/.claude/plans/nah-investigate-the-memory-scalable-meteor.md` "Event taxonomy (all 30, v1)" section. Allowlist of attribute keys is enforced by `backend/internal/obs/allowlist_test.go#TestSlogAttrsOnAllowlist` — a new slog call with an unlisted key fails CI.
